 ...
 */
Foundation.Scanner = function (descriptors) {
	/**
	 * The dialect the scanner is currently using.
	 * @private
	 * @type {Foundation.Scanner.Dialect?}
	 */
	this._dialect = null;
	 ...
	 * @private
	 * @type {Object.<string,Foundation.Scanner.Dialect>}
	 */
	this._dialects = {};
	...
	if (descriptors) {
		...
		if (Object.keys(descriptors).length) {
			...
			Foundation.each(descriptors, function (dialect, i, name) {
				this.addDialect(name, dialect);
			}.bind(this));
 ...
 */
Foundation.Scanner.prototype.setSource = function (sourceStr) {
 ...
 * @return {string?}
 */
Foundation.Scanner.prototype.currentDialect = function () {
 ...
 * @return {Array.<string>}
 */
Foundation.Scanner.prototype.dialects = function () {
 ...
 */
Foundation.Scanner.prototype.addDialect = function (name, descriptors) {
	if (this._dialects[name]) {
		throw new Foundation.Scanner.Error("Dialect \""+name+"\" already exists.", this);
	}
 ...
 * @param {string?} name
 */
Foundation.Scanner.prototype.setDialect = function (name) {
	if (name === null) {
		this._dialect = null;
		return;
	}
	...
	if (! this._dialects[name]) {
		throw new Foundation.Scanner.Error("No such dialect: \""+name+"\".", this);
	}
 ...
 * @param {string} name
 */
Foundation.Scanner.prototype.pushDialect = function (name) {
	if (this._dialect) {
 ...
 * @public
 */
Foundation.Scanner.prototype.popDialect = function () {
 ...
 * @return {Array.<string>}
 */
Foundation.Scanner.prototype.dialectStack = function () {
	return this._dialectStack;
};

/**
 * Generate a Foundation.Scanner.Dialect from caller's token
 * descriptions.
 * @private
 * @param {string} name
 * @param {Array.<Object.<string,RegExp>>} descriptors
 * @return {Foundation.Scanner.Dialect}
 */
Foundation.Scanner.prototype.createDialect = function (name, descriptors) {
	...
	for (var i = 0; i < descriptors.length; i++) {
		...
		for (var type in descriptor) {
			...
			if (j >= 1) throw new Foundation.Scanner.Error(
			...
			if (typesSeen[type]) {
				throw new Foundation.Scanner.Error(
			...
			collectedDescriptors.push(
				new Foundation.Scanner.TokenDescriptor(type, descriptor[type])
			);
	...
	return new Foundation.Scanner.Dialect(name, collectedDescriptors);
};

/**
 * Find and return the next token in the source string. Throws a
 * Foundation.Scanner.Error if the scanner can't continue. Returns
 * null when the entire string has been scanned.
 * @public
 * @param {Array.<string>=} expectedTokens
 * @return {Foundation.Scanner.Token}
 */
Foundation.Scanner.prototype.nextToken = function (expectedTokens) {
	// Sanity check to ensure that we have a dialect currently selected.
	if (! this._dialect) {
		throw new Foundation.Scanner.Error("Dialect was not set.", this);
	}

	// Sanity check to make sure caller has set us up with a source string.
	if (this._source === null) {
		throw new Foundation.Scanner.Error("Source string was not set.", this);
	}
	...
	if (expectedTokens) {
		...
		for (var i = 0; i < expectedTokens.length; i++) {
			...
			if (!descriptor) {
				throw new Foundation.Scanner.Error("Unknown descriptor: "+type, this);
			}
	...
	for (var i = 0; i < descriptors.length; i++) {
		...
		if (match) {
			...
			// Create the token!
			token = new Foundation.Scanner.Token(
				descriptor.type,
				match[0],
				this._pos,
				this._line,
	...
	if (! token) throw new Foundation.Scanner.Error("Unexptected character.", this);
 ...
 * @param {Foundation.Scanner} scanner
 */
Foundation.Scanner.Error = function (message, scanner) {
...
Foundation.inherit(Foundation.Scanner.Error, Error);
 ...
 * @param {RegExp} regex
 */
Foundation.Scanner.TokenDescriptor = function (type, regex) {
	this.type = type;
 ...
 * @param {number} column
 */
Foundation.Scanner.Token = function (type, value, index, line, column) {
 ...
 * @private
 * @param {string} name
 * @param {Array.<Foundation.Scanner.TokenDescriptor>} descriptors
 */
Foundation.Scanner.Dialect = function (name, descriptors) {
	/**
	 * @private
	 * @type {string}
	 */
	...
	/**
	 * @private
	 * @type {Array.<Foundation.Scanner.TokenDescriptor>}
	 */
	this._descriptors = descriptors;

	/**
	 * @private
	 * @type {Object.<string,Foundation.Scanner.TokenDescriptor>|null}
	 */
 ...
 * @return {string}
 */
Foundation.Scanner.Dialect.prototype.name = function () {
	return this._name;
};

/**
 * @private
 * @return {Array.<Foundation.Scanner.TokenDescriptor>}
 */
Foundation.Scanner.Dialect.prototype.descriptors = function () {
 ...
 * @protected
 * @param {string} type
 * @return {Foundation.Scanner.TokenDescriptor}
 */
Foundation.Scanner.Dialect.prototype.getDescriptor = function (type) {
